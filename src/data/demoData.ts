...
export function createDemoMarkets(now = Date.UTC(2026, 5, 30, 8, 45)): DemoMarket[] {
  return SYMBOLS.map((item, symbolIndex) => {
    const m5Base = candles(2600, now, 5 * 60 * 1000, item.base, item.trend * 0.12, symbolIndex, true);
    return {
      symbol: item.symbol,
      name: item.name,
      timeframes: {
        daily: enrichWithSyntheticBidAsk(candles(90, now, 24 * 60 * 60 * 1000, item.base, item.trend * 20, symbolIndex), item.symbol),
        h4: enrichWithSyntheticBidAsk(trimCandles(aggregateCandles(m5Base, "4h"), 120), item.symbol),
        h1: enrichWithSyntheticBidAsk(trimCandles(aggregateCandles(m5Base, "1h"), 160), item.symbol),
        m15: enrichWithSyntheticBidAsk(trimCandles(aggregateCandles(m5Base, "15m"), 180), item.symbol),
        m5: enrichWithSyntheticBidAsk(trimCandles(m5Base, 160), item.symbol)
      }
    };
  });
}
